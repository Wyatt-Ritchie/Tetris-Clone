...
#include <time.h>
#include "Math.h"
FlatPiece::FlatPiece(Game* game) : Actor(game)
								, mCenterBlock(nullptr)
								, mSecondLeftBlock(nullptr)
								, mLeftBlock(nullptr)
								, mRightBlock(nullptr)
								, mOrientation(0)
								, mBlocks()
								...
								, moveUsed(false)
{
	...
	mSecondLeftBlock = new Block(game);
	mSecondLeftBlock->SetIsCenter(false);
	mBlocks.push_back(mSecondLeftBlock);
	...
	mSecondLeftBlock->SetPosition(centerPos + Vector2(-64.0f, 0.0f));
...
void FlatPiece::UpdateActor(float deltaTime)
{
...
void FlatPiece::RotateClockwise()
{
	if (mRotUsed == false)
	{
		mRotUsed = true;
		...
		{
		case 90:
			mLeftBlock->SetPosition(centerPos + Vector2(0.0f, -32.0f));
			mSecondLeftBlock->SetPosition(centerPos + Vector2(0.0f, -64.0f));
...
void FlatPiece::MovePiece(float distance)
{
	for (auto& e : GetBlocks())
	{
...
void FlatPiece::ActorInput(const uint8_t* keyState)
{
	if (getState() == EActive)
	{
...
bool FlatPiece::BlockVertCollision(Block* block)
{
	int row, col;
...
bool FlatPiece::BlockHorCollision(Block* block)
{
