...
#include <time.h>
#include "Math.h"
ReverseZPiece::ReverseZPiece(Game* game) : Actor(game)
										...
										, moveUsed(false)
{
	...
	mTopRightBlock->SetPosition(centerPos + Vector2(32.0f, -32.0f));
	...
	std::cout << "RZ piece created." << std::endl;
}


ReverseZPiece::~ReverseZPiece()
{
	
}

bool ReverseZPiece::BlockHorCollision(Block* block)
{
...
void ReverseZPiece::UpdateActor(float deltaTime)
{
...
void ReverseZPiece::RotateClockwise()
{
	if (mRotUsed == false)
	{
		mRotUsed = true;
...
void ReverseZPiece::MovePiece(float distance)
{
	for (auto& e : GetBlocks())
	{
...
void ReverseZPiece::ActorInput(const uint8_t* keyState)
{
	if (getState() == EActive)
	{
...
bool ReverseZPiece::BlockVertCollision(Block* block)
{
	int row, col;
