...
#include <time.h>
#include "Math.h"
ReverseLPiece::ReverseLPiece(Game* game) : Actor(game)
										...
										, moveUsed(false)
{
	...
	std::cout << "RL piece created." << std::endl;
}
...
void ReverseLPiece::UpdateActor(float deltaTime)
{
...
void ReverseLPiece::RotateClockwise()
{
	if (mRotUsed == false)
	{
		mRotUsed = true;
...
void ReverseLPiece::MovePiece(float distance)
{
	for (auto& e : GetBlocks())
	{
...
void ReverseLPiece::ActorInput(const uint8_t* keyState)
{
	if (getState() == EActive)
	{
...
bool ReverseLPiece::BlockVertCollision(Block* block)
{
	int row, col;
...
bool ReverseLPiece::BlockHorCollision(Block* block)
{
